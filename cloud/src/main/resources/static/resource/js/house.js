...
layui.define(['element', 'carousel', 'table', 'util'], function (exports) {
    ...
    table.render({
        ...
        , where: {'status': '待收货'}
        , cols: [[
            {title: '订单信息', align: 'center', templet: '#orderLeaseTpl'}
            , {field: 'avatar', title: '订购商品', templet: '#imgTpl', align: 'center'}
            , {field: 'number', title: '件数', align: 'center', width: 80}
            , {title: '价格', align: 'center', templet: '#priceTpl', width: 100}
            , {title: '订单状态', align: 'center', templet: '#stateTpl', width: 100}
    ...
    table.on('tool(house-user-order)', function (obj) {
        ...
        if (obj.event === 'check') {
        	...
        	layer.open({
                type: 2
                , content: '/front/return.do?productId=' + data.productId + '&orderId=' + data.id
                , area: ['50%', '70%']
                ,
            });
        	...
        	layer.open({
                type: 2
                , content: '/front/lease.do?productId=' + data.productId + '&orderId=' + data.id
                , area: ['50%', '70%']
                ,
            });
    ...
    table.render({
        ...
        , cols: [[
            ...
            , {title: '租赁时间(天)', align: 'center', width: 150, templet: '#leaseTpl'}
